...
- `edit-selection.ts` — Inline-edit (Track A) helpers: `buildEditPrompt(selection, instruction)` returns the system + user message bodies; `extractRewrite(modelText)` strips a single wrapping code fence if the model ignored the "no fence" instruction; `runEditRequest(plugin, selection, instruction, signal)` makes a one-shot provider call (no tools, no chain, no caching) and returns the rewrite. Uses `plugin.settings.defaultModelRef` — a separate edit model setting was deliberately deferred to v1.1.
- `modal-edit-selection.ts` — `EditSelectionModal`. State machine (idle / streaming / done / error) wrapping the runEditRequest call. Idle: instruction textarea + Edit button. Streaming: locked input, "Thinking…", Stop button (AbortController). Done: unified diff via `lineDiff` from view-helpers, Accept / Reject. Error: message + Retry. Closing the modal aborts in-flight requests. Cmd/Ctrl+Enter submits from idle.
...
- **Inline edit ("Edit with AI").** Sidebar chat is no longer the only invocation surface — users can now edit selections directly in the active editor. Two entry points: command palette / `Cmd+K` hotkey, or right-click in the editor (long-press on mobile) → "Edit with AI" when a selection exists. Opens `EditSelectionModal`: instruction input → one-shot provider call against the configured chat model → unified diff (Accept / Reject). Accept calls `editor.replaceSelection(newText)` so Obsidian's native undo recovers the original. Aborting (Stop button, Esc, or closing the modal) cancels the in-flight request. The modal flow is foreground — it deliberately does NOT go through the per-write approval card (the chat approval card guards async assistant-initiated writes; the inline edit is the user's own foreground action). No separate `editModelRef` setting in v1; one was designed but deferred until real demand. Streaming UX is buffered (the full rewrite arrives before the diff renders) — edits are short enough that latency-to-first-token isn't worth the diff-flicker tradeoff.
