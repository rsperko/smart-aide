...
- `picker-models.ts`, `picker-notes.ts`, `picker-skills.ts` — `FuzzySuggestModal` subclasses for model picking, the full-vault `@`-mention picker (pins on select; see Composer for semantics), and the `/`-slash skill picker (only lists skills with `user-invocable: true`).
...
- **Composer.** Enter or Shift+Enter both send on desktop; mobile tap-Send. Textarea starts at 1 row and grows on input. `@` opens a full-vault fuzzy picker that **pins** the selected note as context (not insert wikilink — that semantic moved to Obsidian's native `[[` picker; one-time tooltip on first use). `/` at the start of an empty composer opens the skill picker (`SkillPickerModal` over `userInvocableSkills()`); selecting fills the composer with `/<name> ` ready for the user to type their prompt. Unknown `/foo` slashes fall through and send verbatim — no error. Drag-dropping a vault `.md` path into the composer or the context strip also pins. Thinking dots while waiting first token. Pencil on every past user message → enters edit-and-fork mode (composer loads with that text, a "Editing — Send to fork / Cancel" banner appears, the message + everything after it disappears from the rendered chain). Send creates a sibling with the original's parentId; the original branch stays in the JSONL but drops out of the active leaf.
...
- **Empty state.** Three-line teaching block: capabilities sentence, `📌 Pinned: <files>` row when any are auto-pinned, `🧠 N skills available · vault context loaded` when applicable. Sets the trust frame before turn 1.
...
- **Top bar.** Chat title on the left — click to switch chats, long-press / right-click to rename. `+` icon on the right for new chat. The Obsidian tab header (above the in-view bar) always reads "Smart Aide" via `getDisplayText()` — brand identity stays at the tab level, the in-view title is just the current chat name. Force-instantiated on plugin load so the panel picker shows the proper icon + label instead of Obsidian's deferred-view ghost placeholder.
