...
- `context-pins.ts` — `PinnedContext`. In-memory pinned-context list for the active chat. Three pin variants in a discriminated union: **file** (vault `.md` path; re-read on every turn so edits show up immediately, cached by mtime+size), **url** (extracted web page content, cached at pin time), **youtube** (caption transcript, cached at pin time). Each pin contributes one section to the user-message preamble; per-source cap mirrors `read_note` auto-truncation.
- `url-extract.ts` — pure-ish helpers + Obsidian-`requestUrl`-backed fetchers for URL pins. `classifyUrl(raw)` distinguishes web pages, YouTube videos (multiple URL shapes — `watch?v=`, `youtu.be`, `/shorts/`, `/embed/`), and rejects non-http(s) schemes. `fetchWebPage` uses the browser-native `DOMParser` (no Readability dep — the model reads raw words, not pretty articles), picks `<article>` → `<main>` → `[itemprop="articleBody"]` → `<body>`, strips script/style/nav/header/footer/aside, returns title + plaintext content + optional byline. `fetchYouTube` scrapes `ytInitialPlayerResponse` from the watch page, follows the captions track URL, parses the timed XML transcript. Failures surface explicitly (no silent empty content) so the user knows when to find a different source. Mobile-safe via `requestUrl`.
- `modal-url-pin.ts` — `UrlPinModal`. Prompted by the `+ URL` button in the context strip. Inline status text replaces the input while fetching; errors stay visible. Routes web URLs to `fetchWebPage` and YouTube URLs to `fetchYouTube`.
- `storage.ts` — Pi session format v3 reader/writer. JSONL files under `<vault>/{metaDir}/Smart Aide/chats/` (default `Meta/Smart Aide/chats/`). Lazy file creation (no file until the first `appendEntry`). `cleanupEmptyChats` sweeps on plugin load. `deleteChat(path)` removes a single session file (called by the picker's × button). Entry IDs are 8-char hex; active leaf tracked implicitly via parent-id walks.
- `tools.ts` — Tool registry: `search_vault`, `read_note`, `list_recent`, `get_backlinks`, `write_note`, `append_to_note`, `delete_note`, `load_skill`, `save_memory`. Path-allowlist guard blocks `.obsidian/` and the entire `{metaDir}/Smart Aide/` subtree (chats, memory.md, `.internals/`) plus absolute / parent-relative — guard reads `metaDir` from `ToolContext` so the forbidden prefixes track whatever the user configures. `read_note` and all write/delete tools enforce `.md` extension (Obsidian's only first-class file type for notes; attachments come in via the paperclip/paste path). `write_note` strips a leading `# <Filename>` line that duplicates Obsidian's inline title. `save_memory` bypasses the path guard (writes only to `memory.md` via its own code path) and carries the fixed section taxonomy + dated-bullet format described under **Persistent memory** below.
...
- `settings.ts` — `SmartAideSettings` shape, legacy-schema migration, `DEFAULT_SYSTEM_PROMPT`, `metaDir`/`pluginHomeFor`/`chatsDirFor`/`skillsDirFor`/`internalDirFor`/`memoryFileFor` helpers, `resolveModelRef`. Pure types/data — no UI. Path helpers split into two families: **cross-tool standards** (`skillsDirFor` → `${metaDir}/skills`, `AGENTS.md` at the metaDir root) and **plugin-only state** (`chatsDirFor` / `internalDirFor` / `memoryFileFor` all under `${metaDir}/Smart Aide/`).
...
- `skills.ts` — `SkillRegistry`. Reads from a single vault-relative directory (default `Meta/skills/`, derived from `settings.metaDir`). Same path on desktop and mobile — no Node-fs fallback. Exposes a manifest for the system prompt. Frontmatter parser supports the Anthropic Agent Skills cross-tool standard fields: required `name` + `description`, optional `mobile`, optional `user-invocable: true` (surfaces as `/<name>` in the composer — our extension), optional `disable-model-invocation: true` (spec field; hides the skill from the manifest and refuses `load_skill(name)` — orthogonal to `user-invocable`; setting both yields the "command-like" pattern: only summonable as `/<name>`, never auto-loaded), optional `allowed-tools: [...]` (scopes the tool registry for the invocation turn). `allowed-tools` accepts both flow `[a, b]` and block `\n  - a\n  - b` styles.
- `agents-md.ts` — `AgentsMdRegistry`. Reads up to two `AGENTS.md` files ([agents.md](https://agents.md/) cross-tool standard): `<vaultRoot>/AGENTS.md` (the location other tools — Pi, Claude Code, Codex — already use) and `${metaDir}/AGENTS.md` (plugin-specific augmentation). Outer first, inner second, concatenated with a horizontal rule and per-file headers so the closer file wins on overlap. Either or both may be absent. If `metaDir` resolves to the vault root, the file is read once. Appended to the system prompt by `composeSystemPrompt()` between the base prompt and the skill manifest, so the model gets vault-specific context (layout, tags, projects, paths to avoid) before the skill catalog. Auto-reloaded by the vault-watcher in `main.ts` on edit / create / delete / rename of either path.
- `edit-selection.ts` — Inline-edit (Track A) helpers. `injectSelectionMarkers(doc, from, to)` brackets the selection with `<<<SMART_AIDE_SELECTION>>>` / `<<<END_SMART_AIDE_SELECTION>>>` at the right character offsets. `capDocumentForWire(marked)` trims around the markers when the doc exceeds 60KB so token budget stays sane. `buildEditPrompt(input)` returns the system + user message bodies — system explains the markers and pins "return only the replacement"; user concatenates Vault context (AGENTS.md), Persistent memory (memory.md), the marked document, and the user's instruction. `extractRewrite(modelText)` strips a single wrapping code fence if the model ignored the "no fence" instruction. `runEditRequest(plugin, modelRef, input, signal)` makes a one-shot provider call (no tools, no chain history, no caching) and returns the rewrite.
- `modal-edit-selection.ts` — `EditSelectionModal`. State machine (idle / streaming / done / error) wrapping the runEditRequest call. Idle: instruction textarea + Edit button + model chip. Streaming: locked input, "Thinking…", Stop button (AbortController). Done: unified diff via `lineDiff` from view-helpers, Accept / Reject. Error: message + Retry. Model chip in the action bar opens `FavoritesPickerModal` to override the model used for the edit (defaults to `defaultModelRef`); favorite toggling is intentionally a no-op here since favorites are managed in settings. Closing the modal aborts in-flight requests. Cmd/Ctrl+Enter submits from idle. The stored selection field is `originalText`, not `selection`, to avoid the `Modal.selection` collision.
- `memory.ts` — `MemoryRegistry`. Reads the model-curated `${metaDir}/Smart Aide/memory.md` file into the system prompt. Counterpart to `AgentsMdRegistry`: AGENTS.md is user-authored vault context, `memory.md` is model-authored persistent facts (with per-write approval through the standard batched-write card). Plain markdown with fixed `##` section headers — see **Persistent memory** below. Loaded inside `onLayoutReady`, auto-reloaded by the vault-watcher in `main.ts` when the file is edited / created / deleted / renamed, reloaded by the Reload button alongside skills/AGENTS, and on `metaDir` change.
...
- **Storage layout.** Two ownership zones inside the user's metaDir (default `Meta/`):
  - **Cross-tool standards** at the metaDir root — `${metaDir}/skills/<name>/SKILL.md` and `${metaDir}/AGENTS.md`. Generic names because they're the published cross-tool conventions (Anthropic Agent Skills + agents.md). Power users symlink these to `~/.agents/skills/` and `~/.agents/AGENTS.md` so the same files serve Pi, Claude Code, Codex.
  - **Plugin-only state** under `${metaDir}/Smart Aide/` — `chats/`, `memory.md`, `.internals/`. Branded subfolder so the vault's file tree visibly distinguishes plugin storage from the user's notes. The path guard treats the whole subtree as off-limits to the general read/write/delete tools.

  Plus the cross-tool standard `<vaultRoot>/AGENTS.md` at the vault root (also read in, concatenated with the metaDir one).
- **9 tools.** Reads: `search_vault` (word-tokenized BM25 over filename + headings + tags, opt-in `deepSearch` extends to note bodies with word-boundary phrase matching; surfaces are fused via Reciprocal Rank Fusion with `k=60`; an auto fuzzy-character-order pass fires when the BM25 path returns nothing — response flags `fuzzyFallback=true` so the model treats those hits as approximate. Lucene defaults `k1=1.2`/`b=0.75`. See `src/tools.ts:12-29` for tuning constants), `read_note` (range / section / auto-truncate, fuzzy section), `list_recent`, `get_backlinks`, `load_skill`. Writes: `write_note`, `append_to_note`, `delete_note`, `save_memory`. Writes in one turn batch into a **single** approval card with per-item checkboxes ("Approve selected" / "Reject all" + an optional "auto-approve future writes in this turn" toggle). Deletes still confirm one-at-a-time (one wrong delete > five wrong appends). `settings.autoApproveWrites` (dangerous mode) bypasses the card for write/append/save_memory; delete still confirms. Approval decisions persist as `custom` entries in the JSONL — the audit trail. `write_note` post-processes content to strip a leading `# <Filename>` line that duplicates Obsidian's inline title (applied in both preview and execute so the diff matches).
- **Skills.** Single vault directory (`Meta/skills/` by default, derived from `settings.metaDir` — common alternative `sys/skills/`). Manifest of skill descriptions injected into the cached system prompt; `load_skill(name)` pulls the body on demand and records it as a `custom_message` entry so the chat history shows what the model saw. Don't splice bodies into the system prefix — it breaks prompt caching. Two invocation paths:
  ...
  - **Suppressing auto-load** (`disable-model-invocation: true`): the spec field for "user-only" — the skill is omitted from the manifest and `load_skill(name)` refuses it, so the model can't auto-load or guess-name into it. Orthogonal to `user-invocable`. The common case is to set BOTH on a "command-like" skill (e.g. `/polish-editor`) that the user always wants to summon explicitly. The user-invocation path (`getByName` + slash picker) is unaffected.
- **Vault context (AGENTS.md).** Reads `<vaultRoot>/AGENTS.md` (cross-tool standard location — Pi, Claude Code, Codex all look here) and `${metaDir}/AGENTS.md` (plugin-specific augmentation). Both optional; if both exist, root first then metaDir, separated by a horizontal rule with file headers so the model can tell them apart. Appended to the system prompt between the base prompt and the skill manifest. Purpose: user-maintained vault context — folder layout, tag conventions, ongoing projects, paths to leave alone. Auto-reloaded by the vault-watcher in `main.ts` whenever either file is edited / created / deleted / renamed; also reloaded by the Reload button alongside skills and on `metaDir` change.
- **URL / YouTube pinning.** Two entry points: paste a URL into the composer and an inline popover (`vk-url-popover`) offers to pin it; or click the `+ URL` button to type a URL into `UrlPinModal`. Both flows route through the same extractors. For a web URL, `requestUrl` fetches the HTML and the custom extractor (browser-native `DOMParser` + element-priority + script/style stripping) returns plaintext content. For a YouTube URL, the watch page is scraped for `ytInitialPlayerResponse`, the captions track URL is followed, and the timed XML transcript is parsed to plaintext. Content is cached at pin time (not re-fetched per turn) and lives in the pin record itself; if the user wants fresh content, they unpin + re-pin. No new runtime deps — we deliberately skipped `@mozilla/readability` because the model reads raw words, not pretty articles, and the custom ~80-line extractor is enough for context-pin purposes.
- **Inline edit ("Edit with AI").** Sidebar chat is no longer the only invocation surface — users can edit selections directly in the active editor. Two entry points: command palette / `Cmd+K` hotkey, or right-click (long-press on mobile) → "Edit with AI" when a selection exists. Opens `EditSelectionModal`: instruction input → one-shot provider call → unified diff (Accept / Reject). Accept calls `editor.replaceSelection(newText)` so Obsidian's native undo recovers the original. Aborting (Stop button, Esc, or closing the modal) cancels the in-flight request. The modal flow is foreground — it deliberately does NOT go through the per-write approval card (the chat approval card guards async assistant-initiated writes; the inline edit is the user's own foreground action). Streaming UX is buffered — edits are short enough that latency-to-first-token isn't worth the diff-flicker tradeoff. **Context sent to the model:** the system prompt + AGENTS.md + memory.md + the **full document** (with the selection bracketed by `<<<SMART_AIDE_SELECTION>>>` markers), plus the user's instruction. Documents over 60KB get trimmed symmetrically around the selection to a ~25KB window. **Model:** chip in the modal's action bar shows the current model and opens the favorites picker on click — defaults to `defaultModelRef` but the user can override per-edit. **Field-name footgun:** Obsidian's `Modal` base class has a `selection: Selection` property of its own; using `private selection` on a subclass gets clobbered by `Modal.open()` before `onOpen` runs. We use `private originalText` instead.
- **Paste-detect popover.** Mirrors the slash-autocomplete popover state machine in `ComposerController`. On every composer `input` event, `parseUrlCandidate(text, cursor)` checks whether the caret sits at the end of an `https?://…` token (whitespace immediately before the cursor dismisses, so settling the URL with a space closes the popover). When a candidate is found, a single inline row appears above the composer: "Pin web page" / "Pin YouTube transcript" with the URL beneath. Enter (with focus in the textarea) or mousedown commits → fetch + pin → strip the URL from the textarea (eating one surrounding space if present so we don't leave a double-space). Esc dismisses and leaves the URL in the textarea so the user keeps their work. Fetch errors stay in the popover with the URL untouched. The URL popover and slash popover are mutually exclusive by their patterns; `updateUrlPopover` defensively closes the slash popover when it activates.
- **Persistent memory (`${metaDir}/Smart Aide/memory.md`).** Model-curated counterpart to AGENTS.md: facts the user told the assistant across conversations that should outlive the current chat. The `save_memory(section, content)` tool appends a dated bullet under one of six fixed sections — `Profile`, `Preferences`, `Decisions`, `Projects`, `People`, `References` — routed through the standard batched-write approval card. The tool prepends today's date automatically (relative-dates-rot is the #1 maintenance failure in industry memory systems); bullets follow a `**Why:** … **How to apply:** …` pattern so future-you can judge edge cases. `save_memory` bypasses the standard `pathGuard` (writes only to `memory.md` via its own code path); the path guard blocks every other tool from touching the file, so the model can't end-run the section taxonomy through `write_note`. The body is loaded by `MemoryRegistry` and injected into the system prompt **between** AGENTS.md and the skill manifest (so order is base → AGENTS.md → memory → skill catalog). Append-only by design — the user prunes by opening the file directly in Obsidian (chip click in the context strip jumps to it); the vault-watcher in `main.ts` picks up edits / deletions immediately so the next turn sees the pruned body. No `update_memory` / no auto-merge / no background distillation pass; if a fact changes, the model writes a new entry and the user reconciles. The memory chip in the context strip is `🧠 Memory` (alongside the AGENTS badge), shown only when the file is non-empty. Each `save_memory` call surfaces in the activity chip as `🧠 Remembered in <Section> — "…"`. Boundary: AGENTS.md is for things the user authors about *their vault* (folder layout, conventions, paths); memory.md is for things the model picks up about *the user* (preferences, decisions, projects). Don't save anything derivable from the vault, in AGENTS.md, or from the current turn's task state.
...
- **Unified context strip** (above the textarea). Holds pinned-context chips (file = file-text icon, web URL = link icon, YouTube = play-circle icon — each with token count, × to unpin), loaded-skill chips (🧠 click to open the skill file), a memory chip (🧠 `Memory`, click to open `memory.md`), an AGENTS badge (📚 when `<vault>/AGENTS.md` is loaded), an `unpin all` button when 3+ pins, and two add buttons: `+ Note` (full-vault fuzzy picker) and `+ URL` (modal that fetches + extracts a web page or YouTube transcript). Clicking a URL/YouTube chip opens the source in the browser. The model sees the same context the user sees in this strip.
...
- **Empty state.** Two-line teaching block: capabilities sentence + `🧠 N skills available · vault context loaded · memory loaded` row when applicable. Pins are intentionally NOT listed here — the context strip above the composer is the source of truth for pins, so duplicating them risks going stale on unpin. Sets the trust frame before turn 1.
...
- **Settings.** Seven sections in order: Endpoints (compact rows; Edit opens an in-tab sub-page, not a modal — `editingEndpointId` state in `SmartAideSettingsTab`), Models (default chat + title; title collapses to "Same as chat model" when mirrored), Storage (Meta folder — single vault-relative path; cross-tool standards `skills/` and `AGENTS.md` sit directly under it, plugin-only state `chats/`, `memory.md`, `.internals/` nest under a `Smart Aide/` subfolder; default `Meta`, common alternative `sys`), Skills (Reload button reloads skills + AGENTS.md + memory), Sample skills (one-click install of bundled starters from `src/sample-skills/` into the user's skills folder, with per-skill state: not installed / installed / modified — "modified" shows an Open link to the user's edited copy), Approvals (auto-approve writes toggle), System prompt (collapsed by default). Empty-state "Get started" card and endpoint-aware "Get a key" link on first install.
...
- **Storage layout = cross-tool standards at the root, plugin-only state under `Smart Aide/`.** The metaDir root is reserved for files that participate in published cross-tool conventions: `skills/<name>/SKILL.md` (Anthropic Agent Skills standard) and `AGENTS.md` (agents.md standard). Anything Smart Aide owns end-to-end — `chats/`, `memory.md`, `.internals/` — nests inside `${metaDir}/Smart Aide/` so the vault file tree visibly tells the user "this is the plugin's storage" vs "these are my notes." Don't mix the two zones (e.g. don't move skills into the Smart Aide subfolder; that breaks symlink sharing). The path guard treats the whole `Smart Aide/` subtree as off-limits to general tools — `save_memory` is the only writer of `memory.md`, and it bypasses the guard via its own code path.
- **Skills live in the vault** (`Meta/skills/` by default — at the metaDir root, NOT under the Smart Aide subfolder, because they're a cross-tool standard). Same path on desktop and mobile. Power users who want to share skills with Pi or Claude Code symlink `~/.agents/skills/` → the vault skills dir themselves — the plugin never reaches outside the vault.
- **Memory is append-only, no background distillation.** `save_memory` only appends a dated bullet to one of six fixed sections; there is no `update_memory`, no auto-merge, and no 24h distillation pass (Claude's pattern). Industry research and our own auto-memory experience converge on the same answer: model-driven rewrites of an established memory file are a footgun — contradictions stack up, the user prunes, and the *user's* contradictions are the signal the model needs to see anyway. Don't add a rewrite path without an explicit user ask.
- **Vault APIs are not ready in `onload()`.** `app.vault.getAbstractFileByPath` / `getFileByPath` / `getFiles` / `getMarkdownFiles` and `app.metadataCache.*` all return null/empty until Obsidian finishes indexing the vault. Plugin init that touches the vault MUST run inside `app.workspace.onLayoutReady(...)` — this is where `skills.load()`, `agents.load()`, and `memory.load()` fire. Race symptom: silently-empty registries on desktop (faster onload than vault index), works on mobile (vault index races ahead of slower onload). Calls from user actions and from tool/turn execution are safe because they happen well after layout-ready by definition.
